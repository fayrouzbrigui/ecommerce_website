...
export class AddProductComponent implements OnInit{
  ...
  constructor(private fb: FormBuilder, private productservice : ProductService, private optionservice: FilterOptionService, private router:Router, private categoryService : CategoriesService, private brandservice: BrandService){
  ...
  postdata(){
    ...
    formData.append('deadline', this.angForm.get('deadline')?.value);
    this.selectedfilteroptionsIds.forEach(id => {
      formData.append('filteroption_id', id.toString());
    });
    
    this.productservice.AddProduct(token, formData).subscribe(data => {
